...
def construct_input(input_shape):
    ...
    rot1 = torch.eye(4).float().cuda().view(1, 4, 4)
    rot1 = torch.cat([rot1 for _ in range(6)], axis=0).view(1, 6, 4, 4)

    input = dict(img_inputs=[
        torch.ones(()).new_empty((1, 6, *input_shape)).cuda(), rot,
        torch.ones((1, 6, 3)).cuda(), rot, rot,
        torch.ones((1, 6, 3)).cuda(),
        torch.eye(3).float().cuda().view(1, 3, 3), rot, rot, torch.ones(()).new_empty((1, 6, *input_shape)).cuda(), torch.ones(()).new_empty((1, 6, *input_shape)).cuda(), rot1,
    ])
    return input
