...
class MEGVIIEMAHook(Hook):
    ...
    def after_train_iter(self, runner):
        ...
        # iteration += 1
        # if iteration % 50 == 0:
        #      self.save_checkpoint_iteration(runner, iteration)
    ...
    @master_only
    def save_checkpoint_iteration(self, runner, iteration):
        state_dict = runner.ema_model.ema.state_dict()
        ema_checkpoint = {
            'epoch': runner.epoch,
            'state_dict': state_dict,
        ...
        save_path = f'epoch_{runner.epoch+1}_{iteration}_ema.pth'
        save_path = os.path.join(runner.work_dir, save_path)
        torch.save(ema_checkpoint, save_path)
        runner.logger.info(f'Saving ema checkpoint at {save_path}')
