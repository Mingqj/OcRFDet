...
@PIPELINES.register_module()
class PrepareImageInputs(LoadImageFromFile):
    """Load multi channel images from a list of separate channel files.
    ...
    def sample_augmentation(self, H, W, flip=None, scale=None):
        ...
        if self.is_train:
            ...
            # resize += np.random.uniform(*self.data_config['resize_test'])
    ...
    def get_sensor2ego_transformation(self,
        ...
        camtoworlds = sweepego2global @ sweepsensor2sweepego
        ...
        keyego2global[:3, :3] = keyego2global_rot
        keyego2global[:3, -1] = keyego2global_tran
        global2keyego = keyego2global.inverse() # ego2global
        ...
        keyego2keysensor = keysensor2keyego.inverse() # cam2ego
        keysensor2sweepsensor = (
            keyego2keysensor @ global2keyego @ sweepego2global
            @ sweepsensor2sweepego).inverse()
        return sweepsensor2keyego, keysensor2sweepsensor, camtoworlds
    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        for cam_name in cam_names:
            ...
            sensor2keyego, sensor2sensor, camtoworlds = \
            ...
            img_wo_aug = img.resize(resize_dims).crop(crop)
            ...
            imgs_wo_aug.append(self.normalize_img(np.array(img_wo_aug)))
            if flip:
                img_wo_aug = img_wo_aug.transpose(method=Image.FLIP_LEFT_RIGHT)
            imgs_wo_norm.append(torch.tensor(np.array(img_wo_aug)).float().permute(2, 0, 1).contiguous())
            ...
            if self.sequential:
                ...
                for adj_info in results['adjacent']:
                    ...
                    img_adjacent_wo_aug = Image.open(filename_adj)
                    img_adjacent = self.img_transform_core(
                        ...
                        flip=flip,
                        rotate=rotate)
                    img_wo_aug =img_adjacent_wo_aug.resize(resize_dims).crop(crop)
        ...
        if self.sequential:
            for adj_info in results['adjacent']:
                ...
                cam2worlds_adj = []
                for cam_name in cam_names:
                    adjsensor2keyego, sensor2sensor, camtoworlds = \
                    ...
                    cam2worlds_adj.append(camtoworlds)
                    sensor2egos.append(sensor2ego)
                    ego2globals.append(ego2global)
                rots.extend(rots_adj)
                trans.extend(trans_adj)
                sensor2sensors.extend(sensor2sensors_adj)
                cam2worlds.extend(cam2worlds_adj)
        imgs = torch.stack(imgs)
        sensor2egos = torch.stack(sensor2egos)
        ego2globals = torch.stack(ego2globals)
        imgs_wo_norm = torch.stack(imgs_wo_norm)
        imgs_wo_aug = torch.stack(imgs_wo_aug)
        ...
        cam2worlds = torch.stack(cam2worlds, axis=0).numpy()
        c2w = cam2worlds.copy()
        trans_center = cam2worlds[len(cam2worlds)//2,:,:].copy()
        P = np.eye(3)
        c2w[:,:3,3] = cam2worlds[:,:3,3]-trans_center[:3,3]
        c2w[:,:3,3] = c2w[:,:3,3]@ trans_center[:3,:3] @ P
        c2w[:,:3,:3] = np.moveaxis(np.dot(np.linalg.inv(trans_center[:3,:3]@P),c2w[:,:3,:3] @ P),1,0)
        c2w = torch.tensor(c2w.copy())
        ...
        return (imgs, rots, trans, intrins, post_rots, post_trans, imgs_wo_norm, imgs_wo_aug, c2w, sensor2egos, ego2globals)
...
class LoadAnnotationsBEVDepth(object):
    ...
    def __call__(self, results):
        ...
        gt_boxes, gt_labels = torch.tensor(np.array(gt_boxes)).to(torch.float32), torch.tensor(np.array(gt_labels))
        ...
        imgs, rots, trans, intrins = results['img_inputs'][:4]
        post_rots, post_trans, imgs_wo_norm, imgs_wo_aug, c2w, sensor2egos, ego2globals = results['img_inputs'][4:]
        results['img_inputs'] = (imgs, rots, trans, intrins, post_rots,
                                 post_trans, bda_rot, sensor2egos, ego2globals, imgs_wo_norm, imgs_wo_aug, c2w)
        return results
