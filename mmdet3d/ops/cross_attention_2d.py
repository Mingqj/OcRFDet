...
class DeformableAttention2D(nn.Module):
    ...
    def forward(self, x_q, x_kv, return_vgrid = False):
        """
        b - batch
        h - heads
        x - height
        ...
        heads, b, h, w, downsample_factor, device = self.heads, x_q.shape[0], *x_q.shape[-2:], self.downsample_factor, x_q.device
