...
from mmdet3d.models.backbones.vggt.vggt.models.aggregator import Aggregator

class CrossAttentionAlign(nn.Module):
    def __init__(self, res_dim=256, vit_dim=2048, num_heads=8):
        super().__init__()
        self.query_proj = nn.Linear(res_dim, vit_dim)
        self.key_proj   = nn.Linear(vit_dim, vit_dim)
        self.value_proj = nn.Linear(vit_dim, vit_dim)

        self.attn = nn.MultiheadAttention(embed_dim=vit_dim, num_heads=num_heads, batch_first=True)
        self.output_proj = nn.Linear(vit_dim, vit_dim)

    def forward(self, res_feat, vit_feat):
        B, N, C_res, H_res, W_res = res_feat.shape
        _, _, C_vit, H_vit, W_vit = vit_feat.shape

        res_feat = res_feat.view(B*N, C_res, H_res*W_res).permute(0, 2, 1)   # [B*N, HW_res, C_res]
        vit_feat = vit_feat.view(B*N, C_vit, H_vit*W_vit).permute(0, 2, 1)   # [B*N, HW_vit, C_vit]

        Q = self.query_proj(res_feat)   # [B*N, HW_res, C_vit]
        K = self.key_proj(vit_feat)     # [B*N, HW_vit, C_vit]
        V = self.value_proj(vit_feat)   # [B*N, HW_vit, C_vit]

        aligned, _ = self.attn(Q, K, V)  # [B*N, HW_res, C_vit]
        aligned = self.output_proj(aligned)

        aligned = aligned.transpose(1, 2)   # [B*N, C_vit, HW_res]
        aligned = nn.functional.interpolate(
            aligned.view(B*N, C_vit, H_res, W_res),
            size=(H_vit, W_vit),
            mode="bilinear",
            align_corners=False
        )

        aligned = aligned.view(B, N, C_vit, H_vit, W_vit)
        return aligned

@DETECTORS.register_module()
class BEVDetVGGT(CenterPoint):
    ...
    def __init__(self,
                 ...
                 use_grid_mask=False,
                 **kwargs):
        super(BEVDetVGGT, self).__init__(**kwargs)
        ...
        state_dict = torch.load("/opt/data/private/codeN2/DualBEV/mmdet3d/models/backbones/vggt/pretrain_model/model.pt", map_location="cpu")
        new_state_dict = {}
        for k, v in state_dict.items():
            if k.startswith("aggregator."):
                new_k = k.replace("aggregator.", "")
                new_state_dict[new_k] = v
        model_dict = self.aggregator.state_dict()
        pretrained_dict = {
            k: v for k, v in new_state_dict.items()
            if k in model_dict and v.shape == model_dict[k].shape
        }
        model_dict.update(pretrained_dict)
        self.aggregator.load_state_dict(model_dict)
        ...
        self.aligner = CrossAttentionAlign(res_dim=512, vit_dim=2048, num_heads=8)
    ...
    def prepare_inputs(self, inputs):
        ...
        imgs, rots, trans, intrins, post_rots, post_trans, bda, sensor2egos, ego2globals = \
            inputs
        ...
        return [imgs, rots, trans, intrins,
                post_rots, post_trans, bda, sensor2keyegos, ego2globals]
...
@DETECTORS.register_module()
class BEVDetTRTVGGT(BEVDetVGGT):
...
class BEVDet4DVGGT(BEVDetVGGT):
    ...
    def __init__(self,
                 ...
                 with_prev=True,
                 **kwargs):
        super(BEVDet4DVGGT, self).__init__(**kwargs)
    ...
    def prepare_inputs(self, inputs, stereo=False):
        ...
        rots, trans, intrins, post_rots, post_trans, bda, sensor2egos, ego2globals = \
            inputs[1:9]

        imgs_wo_norm = inputs[9].view(B, N, self.num_frame, 3, H, W)
        imgs_wo_norm = torch.split(imgs_wo_norm, 1, 2)
        imgs_wo_norm = [t.squeeze(2) for t in imgs_wo_norm]

        imgs_wo_aug = inputs[10].view(B, N, self.num_frame, 3, H, W)
        imgs_wo_aug = torch.split(imgs_wo_aug, 1, 2)
        imgs_wo_aug = [t.squeeze(2) for t in imgs_wo_aug]

        c2w = inputs[11]
        # print(sensor2egos.shape, self.num_frame, N)
        ...
        extra = [
            ...
            c2w.view(B, self.num_frame, N, 4, 4)
        ]
        extra = [torch.split(t, 1, 1) for t in extra]
        extra = [[p.squeeze(1) for p in t] for t in extra]
        rots, trans, sensor2keyegos, ego2globals, intrins, post_rots, post_trans, c2w = extra
        return imgs, rots, trans, intrins, post_rots, post_trans, \
               bda, sensor2keyegos, ego2globals, curr2adjsensor, imgs_wo_norm, imgs_wo_aug, c2w
...
@DETECTORS.register_module()
class BEVDepth4DVGGT(BEVDet4DVGGT):
...
@DETECTORS.register_module()
class BEVStereo4DVGGT(BEVDepth4DVGGT):
    def __init__(self, **kwargs):
        super(BEVStereo4DVGGT, self).__init__(**kwargs)
