...
class DepthNet(nn.Module):
    ...
    def __init__(self,
        ...
        if stereo:
            ...
            downsample = nn.Conv2d(depth_conv_input_channels - 2,
                                    mid_channels, 1, 1, 0)
            self.cost_volumn_net = nn.Sequential(
                nn.Conv2d(depth_channels - 2, depth_channels - 2, kernel_size=3,
                              stride=2, padding=1),
            ...
            self.bias = bias
        depth_conv_list = [BasicBlock(depth_conv_input_channels - 2, mid_channels,
    ...
    def calculate_cost_volumn(self, metas):
        ...
        for fid in range(curr.shape[1] // group_size): # 512, 4, 128
    ...
    def forward(self, x, mlp_input, stereo_metas=None):
        ...
        if not stereo_metas is None:
            if stereo_metas['cv_feat_list'][0] is None:
                ...
                cost_volumn = \
                    torch.zeros((BN, self.depth_channels - 2,
                                 int(H*scale_factor),
